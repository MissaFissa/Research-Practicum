...
class DataCalibration:
    ...
    def __init__(self):
        ...
        self.wavelengths_water = self.df_water[self.dict_water[0]].astype(float)
        self.indices_water = self.df_water[self.dict_water[1]].astype(float)
        self.df_calibration = pd.read_csv(f'{cwd}/data/data_experiment/experiment_calibration.csv', sep=';', skiprows=[0,1,2,3,4], header=[0], decimal=',')
        self.info = pd.read_csv(f'{cwd}/data/data_experiment/experiment_calibration.csv', sep=';', index_col=0, nrows=2, skiprows=[1,4], decimal=',')
        self.dict = dict(enumerate(self.df_calibration))
        ...
        self.water_indices = []
        self.index_wl400nm = 451
        self.wavelengths = self.df_calibration[self.dict[0]]
        self.wavelengths_400nm = self.df_calibration[self.dict[0]][self.index_wl400nm:]
        self.t_reference = 6
        self.NA = 0.39
        self.n_water = 1.33
        self.n_air = 1.0
    ...
    def split_dataframe(self):
        ...
        for k, v in self.dict.items():
            ...
            if '200' in v:
                ...
                dict_200_temp.update({v:self.df_calibration[v]})
                t_200_temp.append(self.t[k-1])

            if '400' in v:

                dict_400_temp.update({v:self.df_calibration[v]})
...
class DataExperiment:
    ...
    def background_correction(self):
        ...
        self.corrected_400_experiment = np.array(self.corrected_400_experiment)
        self.corrected_200_experiment = np.array(self.corrected_200_experiment)
